...
export type ReceiptPrinterStage =
  "processing" | "printing" | "complete" | "error" | "pending"
      ...
      <AnimatePresence initial={false} mode="sync">
        {stage === "complete" ? (
          <motion.span
            ...
            className="text-green-9 col-start-1 row-start-1 grid place-items-center"
            exit={{
              opacity: animate ? 0 : 1,
          ...
          <motion.span
            animate={{ opacity: 1, transform: "scale(1)" }}
            className="col-start-1 row-start-1 grid place-items-center text-berry-500"
            exit={{
              opacity: animate ? 0 : 1,
              transform: move ? "scale(0.96)" : "scale(1)",
            }}
      ...
      >
        <AnimatePresence initial={false} mode="sync">
          <motion.div
            ...
            className={cn(
              "col-start-1 row-start-1 truncate text-xs leading-none font-medium",
              stage === "error"
                ? "text-berry-400"
                : "text-grayscale-8 dark:text-grayscale-11"
            )}
            exit={{
              opacity: animate ? 0 : 1,
...
function ReceiptPrinterPaper({
  ...
  return (
    <article
      className={cn(
        "relative z-10 min-h-80 bg-grayscale-1 bg-[url('/textures/receipt-paper.svg')] bg-cover px-4 pt-7 pb-8 font-sans! text-grayscale-12 bg-blend-soft-light dark:bg-grayscale-12 dark:text-grayscale-1",
        className
      )}
...
function ReceiptPrinterOutput({
  ...
  const isStatic =
    stage === "complete" || stage === "pending" || stage === "error"

  // Overflow stays clipped until the slide-in transform finishes —
  // otherwise the un-clipped overshoot paints the paper over the machine.
  const [settled, setSettled] = useState(isStatic && !shouldMove)

  useEffect(() => {
    if (!isStatic) {
      setSettled(false)
    } else if (!shouldMove) {
      setSettled(true) // reduced motion / animate off — nothing to wait on
    } else {
      // Fallback timer for direct-mount as complete (cached query) where
      // onAnimationComplete may not fire reliably due to initial={false}
      const t = setTimeout(() => setSettled(true), 2000)
      return () => clearTimeout(t)
    }
  }, [isStatic, shouldMove])

  const allowOverflow = isStatic && settled

  return (
    <div
      className={cn(
        "relative z-50 -mt-4 w-[calc(80%+3rem)] max-w-full px-6",
        allowOverflow ? "h-auto overflow-visible" : "h-[32rem] overflow-hidden",
        className
      )}
      {...props}
    >
      ...
      <motion.div
        ...
        onAnimationComplete={() => {
          if (isStatic) setSettled(true)
        }}
        aria-hidden={stage !== "complete" && stage !== "error"}
